...
#if XRENDER
using XRender.Pipeline;
using XRender.XShaderDebug;
#endif

namespace Editor.ImpostorGeneration.AmplifyImpostors.Scripts
{
            ...
            if (Activated)
            {
                ...
                if (ImportData.TryGetValue(assetPath, out data))
                {
                    ...
                    textureImporter.alphaSource =
                        data.Alpha ? TextureImporterAlphaSource.FromInput : TextureImporterAlphaSource.None;
    ...
    public class AmplifyImpostor : MonoBehaviour
    {
        ...
        //public const string ShaderOcta = "a09ba80639693944f94e43be95a4946a";
        ...
        //[SerializeField] private string m_shaderGuid = "a09ba80639693944f94e43be95a4946a";
        [SerializeField] private string m_shaderGuid = "5a9cf4a186619d04c94621ae965c3b27";

        public string ShaderGuid
        {
            get { return m_shaderGuid; }
            set { m_shaderGuid = value; }
        }

        [SerializeField] private AmplifyImpostorAsset m_data;
        ...
        public LODReplacement m_lodReplacement = LODReplacement.InsertAfter;

        [SerializeField] public RenderPipelineInUse m_renderPipelineInUse = RenderPipelineInUse.None;

        public int m_insertIndex = 1;

        [SerializeField] public GameObject m_lastImpostor;

        [SerializeField] public string m_folderPath;

        [NonSerialized] public string m_impostorName = string.Empty;

        [SerializeField] public CutMode m_cutMode = CutMode.Automatic;

        [NonSerialized] private const float StartXRotation = -90;
        [NonSerialized] private const float StartYRotation = 90;
        [NonSerialized] private const int MinAlphaResolution = 256;
        [NonSerialized] private RenderTexture[] m_rtGBuffers;
        [NonSerialized] private RenderTexture[] m_alphaGBuffers;
        [NonSerialized] private RenderTexture m_trueDepth;
        [NonSerialized] public Texture2D m_alphaTex;
        [NonSerialized] private float m_xyFitSize = 0;
        [NonSerialized] private float m_depthFitSize = 0;
        [NonSerialized] private Vector2 m_pixelOffset = Vector2.zero;

        [NonSerialized] private Bounds m_originalBound = new Bounds();

        [NonSerialized] private Vector3 m_oriPos = Vector3.zero;

        [NonSerialized] private Quaternion m_oriRot = Quaternion.identity;

        [NonSerialized] private Vector3 m_oriSca = Vector3.one;

        [NonSerialized] private const int BlockSize = 65536;
#if UNITY_EDITOR
        [NonSerialized] private readonly string[] m_propertyNames =
            { "_Albedo", "_ORMask", "_Normals", "_Emission", "_Features" };

        [NonSerialized] private string[] m_standardFileNames =
            { string.Empty, string.Empty, string.Empty, string.Empty, string.Empty };

        [NonSerialized] private string[] m_fileNames =
            { string.Empty, string.Empty, string.Empty, string.Empty, string.Empty };
#endif
            ...
            else
            {
                for (int i = 0; i < m_rtGBuffers.Length; i++)
                {
                    RenderTextureFormat format =
                        outputList[i].SRGB ? RenderTextureFormat.ARGB32 : RenderTextureFormat.ARGBHalf;
                    //if (i == 1)
                    //	format = RenderTextureFormat.R8;
                    m_rtGBuffers[i] = new RenderTexture((int)m_data.TexSize.x, (int)m_data.TexSize.y, 16, format)
                        { name = $"GBuffer {i}" };
        ...
        private void GenerateAlphaTextures(List<TextureOutput> outputList)
        {
            ...
            {
                m_alphaGBuffers[i] =
                    new RenderTexture(MinAlphaResolution, MinAlphaResolution, 16,
                            outputList[i].SRGB ? RenderTextureFormat.ARGB32 : RenderTextureFormat.ARGBHalf)
                        { name = $"Alpha GBuffer {i}" };
        ...
        public void RenderToTexture(ref RenderTexture tex, string path, ImageFormat imageFormat, int resizeScale,
            TextureChannels channels)
        {
            ...
            //Debug.Log("###Tex " + tex.name + "TextureChannels: " + channels);

            if (imageFormat == ImageFormat.EXR)
                outfile = new Texture2D((int)m_data.TexSize.x / resizeScale, (int)m_data.TexSize.y / resizeScale,
                    TextureFormat.RGBAFloat, false);
            else
            {
                outfile = new Texture2D((int)m_data.TexSize.x / resizeScale, (int)m_data.TexSize.y / resizeScale,
                    texFormat, true);
                //Debug.Log("###Tex " + tex.name + "TextureFormat: " + texFormat);
            }

            outfile.name = Path.GetFileNameWithoutExtension(path);
            RenderTexture temp = RenderTexture.active;
            RenderTexture.active = tex;
            outfile.ReadPixels(new Rect(0, 0, (int)m_data.TexSize.x / resizeScale, (int)m_data.TexSize.y / resizeScale),
                0, 0);
            ...
            else
            {
                ...
                FileStream FSFile = new FileStream(path, FileMode.Create, FileAccess.Write, FileShare.None, BlockSize,
                    false);
                BufIndex = 0;
                do
                {
        ...
        public void ChangeTextureImporter(ref RenderTexture tex, string path, bool sRGB = true,
            bool changeResolution = false, TextureCompression compression = TextureCompression.Normal,
            bool alpha = true)
        {
            Texture2D outfile = AssetDatabase.LoadAssetAtPath<Texture2D>(path);
            ...
            if (tImporter != null)
            {
                ...
                {
                    tImporter.sRGBTexture = sRGB;
                    tImporter.alphaSource =
                        alpha ? TextureImporterAlphaSource.FromInput : TextureImporterAlphaSource.None;
                    tImporter.textureCompression = (TextureImporterCompression)compression;
                    if (changeResolution)
                ...
                for (int y = 0; y <= vframes; y++)
                {
                    ...
                    for (int i = 0; i < Renderers.Length; i++)
                    {
                        ...
                        if (frameBounds.size == Vector3.zero)
                            frameBounds = mf.sharedMesh.bounds.Transform(m_rootTransform.worldToLocalMatrix *
                                                                         Renderers[i].localToWorldMatrix);
                        else
                            frameBounds.Encapsulate(mf.sharedMesh.bounds.Transform(m_rootTransform.worldToLocalMatrix *
                                Renderers[i].localToWorldMatrix));
                    }
        ...
        public void DilateRenderTextureUsingMask(ref RenderTexture mainTex, ref RenderTexture maskTex, int pixelBleed,
            bool alpha, Material dilateMat = null)
        {
            if (pixelBleed == 0)
                return;
            ...
            RenderTexture tempTex =
                RenderTexture.GetTemporary(mainTex.width, mainTex.height, mainTex.depth, mainTex.format);
            RenderTexture tempMask =
                RenderTexture.GetTemporary(maskTex.width, maskTex.height, maskTex.depth, maskTex.format);
            RenderTexture dilatedMask =
                RenderTexture.GetTemporary(maskTex.width, maskTex.height, maskTex.depth, maskTex.format);
            ...
            {
                //dilateMat.SetTexture( "_MainTex", mainTex );
        ...
        public void PackingRemapping(ref RenderTexture src, ref RenderTexture dst, int passIndex,
            Material packerMat = null, Texture extraTex = null, string texName = null)
        {
            bool destroyMaterial = false;
            if (packerMat == null)
            {
        ...
        public void CalculatePixelBounds(int targetAmount)
        {
            ...
            try
            {
                RenderImpostor(m_data.ImpostorType, m_data.Preset.Output.Count, false, true, true,
                    m_data.Preset.BakeShader);
                PasteTransform();
            }
            catch (Exception e)
            {
            ...
            if (m_renderPipelineInUse == RenderPipelineInUse.HDRP && standardRendering)
            {
                // getting alpha
                {
                    RenderTexture tempTex2 = RenderTextureEx.GetTemporary(m_alphaGBuffers[0]);
                    Graphics.Blit(m_alphaGBuffers[0], tempTex2);
                    packerMat.SetTexture("_A", tempTex2);
                    Graphics.Blit(m_trueDepth, m_alphaGBuffers[0], packerMat, 11);
            ...
            // Render just alpha
            RenderTexture combinedAlphaTexture = RenderTexture.GetTemporary(MinAlphaResolution, MinAlphaResolution,
                m_alphaGBuffers[alphaIndex].depth, m_alphaGBuffers[alphaIndex].format);
            ...
            Texture2D tempTex = new Texture2D(combinedAlphaTexture.width, combinedAlphaTexture.height,
                TextureFormat.RGBAFloat, false);
            ...
            for (int i = 0; i < paths.Length; i++)
            {
                ...
                {
                    minMaxPoints[index] = (Vector2)(paths[i][j]) +
                                          (new Vector2(tempTex.width * 0.5f, tempTex.height * 0.5f));
                    minMaxPoints[index] = Vector2.Scale(minMaxPoints[index],
                        new Vector2(1.0f / tempTex.width, 1.0f / tempTex.height));
                    index++;
                }
            }
        ...
        public void RenderCombinedAlpha(AmplifyImpostorAsset data = null)
        {
            ...
            try
            {
                RenderImpostor(m_data.ImpostorType, m_data.Preset.Output.Count, false, true, false,
                    m_data.Preset.BakeShader);
                PasteTransform();
            }
            catch (Exception e)
            ...
            if (m_renderPipelineInUse == RenderPipelineInUse.HDRP && standardRendering)
            {
                // getting alpha
                {
                    RenderTexture tempTex = RenderTextureEx.GetTemporary(m_alphaGBuffers[0]);
                    Graphics.Blit(m_alphaGBuffers[0], tempTex);
            ...
            RenderTexture.active = combinedAlphaTexture;
            m_alphaTex = new Texture2D(combinedAlphaTexture.width, combinedAlphaTexture.height, TextureFormat.RGBAFloat,
                false);
        ...
        public void CreateAssetFile(AmplifyImpostorAsset data = null)
        {
            ...
            if (m_data == null)
            {
                ...
                AmplifyImpostorAsset existingAsset =
                    AssetDatabase.LoadAssetAtPath<AmplifyImpostorAsset>(folderPath + fileName + ".asset");
        ...
        public void DetectRenderPipeline()
        {
            ...
            try
            {
                AmplifyImpostorBakePreset check =
                    AssetDatabase.LoadAssetAtPath<AmplifyImpostorBakePreset>(AssetDatabase.GUIDToAssetPath(PresetURP));
                if (check == null && m_renderPipelineInUse == RenderPipelineInUse.URP)
                    missingFiles = true;

                check = AssetDatabase.LoadAssetAtPath<AmplifyImpostorBakePreset>(
                    AssetDatabase.GUIDToAssetPath(PresetHDRP));
        ...
        public void RenderAllDeferredGroups(AmplifyImpostorAsset data = null)
        {
            ...
            fileName = fileName.Replace("Prefab_", "");
            ...
            if (m_data.Preset == null)
            {
                if (m_renderPipelineInUse == RenderPipelineInUse.HDRP)
                    m_data.Preset =
                        AssetDatabase.LoadAssetAtPath<AmplifyImpostorBakePreset>(
                            AssetDatabase.GUIDToAssetPath(PresetHDRP));
                else if (m_renderPipelineInUse == RenderPipelineInUse.URP)
                    m_data.Preset =
                        AssetDatabase.LoadAssetAtPath<AmplifyImpostorBakePreset>(
                            AssetDatabase.GUIDToAssetPath(PresetURP));
                else
                    m_data.Preset =
                        AssetDatabase.LoadAssetAtPath<AmplifyImpostorBakePreset>(AssetDatabase.GUIDToAssetPath(Preset));
            }
            ...
            for (int i = 0; i < m_data.OverrideOutput.Count && i < m_data.Preset.Output.Count; i++)
            {
                ...
                if ((m_data.OverrideOutput[i].OverrideMask & OverrideMask.QualityCompression) ==
                    OverrideMask.QualityCompression)
            ...
            if (m_renderPipelineInUse == RenderPipelineInUse.HDRP)
                ...
                guid = m_data.ImpostorType == ImpostorType.Spherical ? Shader : m_shaderGuid;
            ...
            if (standardRendering)
            {
                ...
                if (m_renderPipelineInUse == RenderPipelineInUse.HDRP)
                {
                    ...
                    // getting alpha
                    {
                        ...
                        Graphics.Blit(m_rtGBuffers[0], tempTex);
                        packerMat.SetTexture("_A", tempTex);
                        Graphics.Blit(m_trueDepth, m_rtGBuffers[0], packerMat, 11);
                        RenderTexture.ReleaseTemporary(tempTex);
                    }
                ...
                {
                    // Switch alpha with occlusion
                    RenderTexture tempTex = RenderTexture.GetTemporary(m_rtGBuffers[0].width, m_rtGBuffers[0].height,
                        m_rtGBuffers[0].depth, m_rtGBuffers[0].format);
                    RenderTexture tempTex2 = RenderTexture.GetTemporary(m_rtGBuffers[3].width, m_rtGBuffers[3].height,
                        m_rtGBuffers[3].depth, m_rtGBuffers[3].format);
            ...
            for (int i = 0; i < outputList.Count; i++)
            {
                if (outputList[i].Active)
                    DilateRenderTextureUsingMask(ref m_rtGBuffers[i], ref m_rtGBuffers[alphaIndex], m_data.PixelPadding,
                        alphaIndex != i, dilateMat);
            }
            ...
            for (int i = 0; i < outputList.Count; i++)
            {
                ...
                {
                    RenderTexture resTex = RenderTexture.GetTemporary(m_rtGBuffers[i].width / (int)outputList[i].Scale,
                        m_rtGBuffers[i].height / (int)outputList[i].Scale, m_rtGBuffers[i].depth,
                        m_rtGBuffers[i].graphicsFormat);
                    ...
                    m_rtGBuffers[i] = new RenderTexture(resTex.width, resTex.height, m_rtGBuffers[i].depth,
                        m_rtGBuffers[i].graphicsFormat);
                    m_rtGBuffers[i].Create();
            ...
            for (int i = 0; i < outputList.Count; i++)
            {
                ...
                else
                {
                    ...
                    m_fileNames[i] += "T_" + fileName + outputList[i].Name + "." +
                                      outputList[i].ImageFormat.ToString().ToLower();
                    //Debug.LogFormat("### folderPath: {0}, fileName: {1}, outputList name: {2}", folderPath, fileName, outputList[i].Name);
                }
            }
            ...
            if (hasDifferentResolution && EditorPrefs.GetInt(Preferences.PrefGlobalTexImport, 0) == 0)
                resizeTextures = EditorUtility.DisplayDialog("Resize Textures?",
                    "Do you wish to override the Texture Import settings to match the provided Impostor Texture Size?",
                    "Yes", "No");
            ...
            if (!Application.isPlaying)
            {
                ...
                {
                    // ToDo
                    // if (!m_fileNames[i].ToLower().Contains("normal") || !m_fileNames[i].ToLower().Contains("albedo"))
                    // 	continue;
                    if (outputList[i].Active)
                    {
                        RenderToTexture(ref m_rtGBuffers[i], m_fileNames[i], outputList[i].ImageFormat,
                            (int)outputList[i].Scale, outputList[i].Channels);
                        AmplifyTextureImporter.ImportData.Add(m_fileNames[i],
                            new DataHolder(outputList[i].SRGB, outputList[i].Compression,
                                outputList[i].Channels == TextureChannels.RGBA,
                                resizeTextures ? (int)m_data.TexSize.x / (int)outputList[i].Scale : -1));
                    }
                }
            }
            ...
            //RenderCombinedAlpha();
            Vector4 offsetCalc = /*transform.worldToLocalMatrix **/ new Vector4(m_originalBound.center.x,
                m_originalBound.center.y, m_originalBound.center.z, 1);
            Vector4 offset = new Vector4(offsetCalc.x, offsetCalc.y, offsetCalc.z,
                -m_pixelOffset.y /
                m_xyFitSize /*(-pixelOffset.y / m_data.VerticalFrames) * ( m_trueFitsize / m_data.VerticalFrames )*/);
            Vector4 sizeOffset = new Vector4(m_xyFitSize, m_depthFitSize,
                (m_pixelOffset.x / m_xyFitSize) / (float)m_data.HorizontalFrames,
                (m_pixelOffset.y / m_xyFitSize) / (float)m_data.VerticalFrames);
            ...
            if (isPrefab)
            {
                if (m_lastImpostor != null &&
                    PrefabUtility.GetPrefabAssetType(m_lastImpostor) == PrefabAssetType.Regular)
                {
                    impostorObject = m_lastImpostor;
                }
                else
                {
                    GameObject mainGO = new GameObject("SM_" + fileName,
            ...
            else
            {
                ...
                {
                    impostorObject =
                        new GameObject("Impostor", new Type[] { typeof(MeshFilter), typeof(MeshRenderer) });
            ...
            if (justCreated)
            {
                if (LodGroup != null)
                {
                    ...
                    {
                        targetPrefab =
                            PrefabUtility.GetPrefabInstanceHandle((Selection.activeObject as GameObject).transform.root
                                .gameObject);
                        GameObject targetGO =
                            AssetDatabase.LoadAssetAtPath(
                                folderPath + (Selection.activeObject as GameObject).transform.root.gameObject.name +
                        ...
                        PrefabUtility.SaveAsPrefabAssetAndConnect(tempGO, AssetDatabase.GetAssetPath(targetPrefab),
                    ...
                    Debug.Log("###Replace Impostor mode: " + m_lodReplacement.ToString());
            ...
            {
                if (outputList[i].Active)
                    ChangeTextureImporter(ref m_rtGBuffers[i], m_fileNames[i], outputList[i].SRGB, resizeTextures,
                        outputList[i].Compression, outputList[i].Channels == TextureChannels.RGBA);
            }
        ...
        public void RenderImpostor(ImpostorType impostorType, int targetAmount, bool impostorMaps = true,
            bool combinedAlphas = false, bool useMinResolution = false, Shader customShader = null)
        {
            ...
            for (int x = 0; x < hframes; x++)
            {
                for (int y = 0; y <= vframes; y++)
                {
                    ...
                    for (int i = 0; i < validMeshesCount; i++)
                    {
                        ...
                        if (frameBounds.size == Vector3.zero)
                            frameBounds = validMeshes[i].sharedMesh.bounds
                                .Transform(m_rootTransform.worldToLocalMatrix * Renderers[i].localToWorldMatrix);
                        else
                            frameBounds.Encapsulate(validMeshes[i].sharedMesh.bounds
                                .Transform(m_rootTransform.worldToLocalMatrix * Renderers[i].localToWorldMatrix));
                    }
                    ...
                    Matrix4x4 V = camMatrixRot.inverse *
                                  Matrix4x4.LookAt(frameBounds.center - new Vector3(0, 0, m_depthFitSize * 0.5f),
                                      frameBounds.center, Vector3.up);
                    float fitSize = m_xyFitSize * 0.5f;
                    Matrix4x4 P = Matrix4x4.Ortho(-fitSize + m_pixelOffset.x, fitSize + m_pixelOffset.x,
                        -fitSize + m_pixelOffset.y, fitSize + m_pixelOffset.y, 0, -m_depthFitSize);
                    ...
                    if (impostorMaps)
                    {
                        ...
                        commandBuffer.SetViewport(new Rect((m_data.TexSize.x / hframes) * x,
                            (m_data.TexSize.y / (vframes + (impostorType == ImpostorType.Spherical ? 1 : 0))) * y,
                            (m_data.TexSize.x / m_data.HorizontalFrames), (m_data.TexSize.y / m_data.VerticalFrames)));
...
#if XRENDER
						var table = new XCamera.XCameraParamTable.PerCameraType();
						table._ViewMatrix = V;
						table._InvViewMatrix = V.inverse;
						table._ProjMatrix = P;
						table._InvProjMatrix = P.inverse;
						table._ViewProjMatrix = P * V;
						table._CameraViewMatrix = V;
						table._WorldSpaceCameraPos = new Vector4(-V.m03, -V.m13, V.m23, 1);
						table._ExposureInfo.x = 1;
						table._ShaderDebugPID = new ParamTable.Types.Int4( (int)XShaderDebugType.ImpostorBake, 
							XShaderDebugUtils.debugMainLight, 
							XShaderDebugUtils.GetDebugMainLightID(), 
							0 );
						ConstantBuffer.PushGlobal<XRender.Pipeline.XCamera.XCameraParamTable.PerCameraType>(commandBuffer, table, UnityEngine.Shader.PropertyToID("_PerCamera"));
#endif
                    }
                    ...
                    if (combinedAlphas)
                    {
						...
						table._ShaderDebugPID = new ParamTable.Types.Int4( (int)XShaderDebugType.ImpostorBake, 
							...
							XShaderDebugUtils.GetDebugMainLightID(), 
							0 );
						ConstantBuffer.PushGlobal<XRender.Pipeline.XCamera.XCameraParamTable.PerCameraType>(commandAlphaBuffer, table, UnityEngine.Shader.PropertyToID("_PerCamera"));
#endif
                    }
                    ...
                    for (int j = 0; j < validMeshesCount; j++)
                    {
                        ...
                        for (int k = 0; k < meshMaterials.Length; k++)
                        {
                            ...
                            if (standardrendering)
                            {
                                ...
                                prePass = renderMaterial.FindPass("DepthNormals");
                            ...
                            else
                            {
                                ...
                                {
                                    renderMaterial = new Material(customShader)
                            ...
                            if ((isUsingBakedGI || isUsingRealtimeGI) && !standardrendering)
                            {
                                ...
                                if (isUsingRealtimeGI)
                                {
                                    ...
                                    commandBuffer.SetGlobalVector("unity_DynamicLightmapST",
                                        Renderers[j].realtimeLightmapScaleOffset);
                                }
                                else
                                {
                            ...
                            commandBuffer.EnableShaderKeyword("_USE_DEBUG_MODE");
                            ...
                            if (impostorMaps)
                            {
                                if (prePass > -1)
                                {
                                    rt[0] = m_rtGBuffers[2];
                                    rt[1] = m_rtGBuffers[1];
                                    commandBuffer.SetRenderTarget(rt, m_trueDepth);
                                    commandBuffer.SetViewport(new Rect((m_data.TexSize.x / hframes) * x,
                                        (m_data.TexSize.y /
                                         (vframes + (impostorType == ImpostorType.Spherical ? 1 : 0))) * y,
                                        (m_data.TexSize.x / m_data.HorizontalFrames),
                                        (m_data.TexSize.y / m_data.VerticalFrames)));
                                ...
                                commandBuffer.SetRenderTarget(rt, m_trueDepth);
                                commandBuffer.SetViewport(new Rect((m_data.TexSize.x / hframes) * x,
                                    (m_data.TexSize.y / (vframes + (impostorType == ImpostorType.Spherical ? 1 : 0))) *
                                    y, (m_data.TexSize.x / m_data.HorizontalFrames),
                                    (m_data.TexSize.y / m_data.VerticalFrames)));
                            ...
                            if (combinedAlphas)
                            {
                                if (prePass > -1)
                                {
                                    rt[0] = m_alphaGBuffers[2];
                                    rt[1] = m_alphaGBuffers[1];
                                    commandAlphaBuffer.SetRenderTarget(rt, m_trueDepth);

                                    commandAlphaBuffer.DrawRenderer(Renderers[j], renderMaterial, k, prePass);
                                }

                                //Forward
                                rt[0] = m_alphaGBuffers[0];
                                rt[1] = m_alphaGBuffers[3];
                                commandAlphaBuffer.SetRenderTarget(rt, m_trueDepth);

                                commandAlphaBuffer.DrawRenderer(Renderers[j], renderMaterial, k, pass);
                            }

                            commandBuffer.DisableShaderKeyword("_USE_DEBUG_MODE");
                        }
                    }
                }
            }
        ...
        private Matrix4x4 GetCameraRotationMatrix(ImpostorType impostorType, int hframes, int vframes, int x, int y)
        {
            ...
            else if (impostorType == ImpostorType.Octahedron) //OCTAHEDRON
            {
                try
                {
                    Vector3 forw = OctahedronToVector(((float)(x) / ((float)hframes - 1)) * 2f - 1f,
                        ((float)(y) / ((float)vframes - 1)) * 2f - 1f);
                    Quaternion octa =
                        Quaternion.LookRotation(new Vector3(forw.x * -1, forw.z * -1, forw.y * -1), Vector3.up);
            ...
            {
                Vector3 forw = HemiOctahedronToVector(((float)(x) / ((float)hframes - 1)) * 2f - 1f,
                    ((float)(y) / ((float)vframes - 1)) * 2f - 1f);
                Quaternion octa =
                    Quaternion.LookRotation(new Vector3(forw.x * -1, forw.z * -1, forw.y * -1), Vector3.up);
                camMatrixRot = Matrix4x4.Rotate(octa).inverse;
        ...
        public void GenerateAutomaticMesh(AmplifyImpostorAsset data)
        {
            ...
            for (int i = 0; i < paths.Length; i++)
            {
                for (int j = 0; j < paths[i].Length; j++)
                {
                    data.ShapePoints[index] = (Vector2)(paths[i][j]) +
                                              (new Vector2(m_alphaTex.width * 0.5f, m_alphaTex.height * 0.5f));
                    data.ShapePoints[index] = Vector2.Scale(data.ShapePoints[index],
                        new Vector2(1.0f / m_alphaTex.width, 1.0f / m_alphaTex.height));
                    index++;
                }
            }
        ...
        public Mesh GenerateMesh(Vector2[] points, Vector3 offset, float width = 1, float height = 1,
            bool invertY = true)
        {
            ...
            for (int i = 0; i < newPoints.Length; i++)
            {
                newPoints[i] = new Vector2(newPoints[i].x * width - halfWidth + m_pixelOffset.x,
                    newPoints[i].y * height - halfHeight + m_pixelOffset.y);
            }
